...
export default function ReimagineStepBar({ currentStep, trailingAction = null }) {
  const active = getReimagineFlowIndex(currentStep);

  return (
    <div className="border-b border-black sticky top-16 z-30 bg-white">
      <div className="tj-container py-4 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 text-xs font-mono-tj uppercase tracking-[0.18em] overflow-x-auto no-scrollbar min-w-0">
        ...
        {trailingAction && (
          <div className="shrink-0 pl-2 border-l border-black/10">{trailingAction}</div>
        )}
      </div>
    </div>
  );
