...
export function AccentPill({ children, color, className = '' }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1 text-xs font-bold uppercase tracking-widest font-[Outfit] ${className}`}
...
export function Badge({ status, className = '' }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold font-[Outfit] tracking-wide uppercase ${className}`}
